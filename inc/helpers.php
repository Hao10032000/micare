...
function themesflat_shortcode_default_id(){
    return array(
                ...
                'group' => esc_html__( 'Design Options', 'micare' ),
...
function themesflat_predefined_header_styles() {
    ...
    if ( empty( $styles ) ) {
        $styles = apply_filters( 'themesflat/header_styles', array(
            'header-v1' => esc_html__( 'Classic', 'micare' ),
            'header-v2' => esc_html__( 'Header style 02', 'micare' ),
            'header-v4' => esc_html__( 'Modern', 'micare' ),
        ) );
    }
...
function themesflat_available_social_icons() {
    $icons = apply_filters( 'themesflat_available_icons', array(
        ...
        'chat'  => array( 'iclass' => 'icon-micare-chat', 'title' => 'Chat','share_link'=> THEMESFLAT_PROTOCOL . '://www.chat.com/share?url=' ),
...
function themesflat_menu_fallback() {
    ...
    <li>
    <a class="menu-fallback" href="' . esc_url(admin_url('nav-menus.php')) . '">' . esc_html__( 'Create a Menu', 'micare' ) . '</a></li></ul>';
}
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
    ...
    function themesflat_wp_title( $title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            $title .= " $sep " . sprintf( esc_html__( 'Page %s', 'micare' ), max( $paged, $page ) );
        }
            ...
            <ul class="unstyled">
                <li class="current">                    
                    <a href="?lang=en" class="lang_sel_sel">
                        <span class="languages-before-icon icon-micare-web"></span><?php echo esc_html__("English",'micare');?><i class="icon-micare-arrow-down" aria-hidden="true"></i>
                    </a>
                    <ul class="unstyled-child">
                       <li class="icl-en">
                            <a href="?lang=en" class="lang_sel_sel">
                             <?php echo esc_html__("English",'micare');?>
                            </a>
                       </li>
                       <li class="icl-fr fr">
                            <a href="?lang=fr" class="lang_sel_other">
                             <?php echo esc_html__("French",'micare');?>
                            </a>
                       </li>
                       <li class="icl-ge ge">
                            <a href="?lang=it" class="lang_sel_other">
                                <?php echo esc_html__("German",'micare');?>
                            </a>
                       </li>
                    </ul>
                </li>
...
function themesflat_custom_search_form( $form ) {
    ...
    <label>
        <span class="screen-reader-text">' . esc_html__( 'Search for:' , 'micare' ) . '</span>
        <input type="search" value="' . get_search_query() . '" name="s" class="s" placeholder="' . esc_html__( "Search…", "micare" ) . '"/>
    </label>
...
function themesflat_get_page_titles() {
    ...
    if ( ! is_archive() ) {       
        ...
        elseif ( is_page() ) {
            ...
            $title = esc_html__( '404', 'micare' );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'micare' ), get_search_query() );
        } else {
            $title = get_post_meta( get_the_ID(), 'custom_title', true );
            if ( ! $title ) {
                $title = get_the_title();
        ...
        if ( is_author() ) {
            the_post();
            $title = sprintf( esc_html__( 'Author: %s', 'micare' ), get_the_author() );
            rewind_posts();
        } elseif ( is_day() ) {
            $title = sprintf( esc_html__( 'Daily: %s', 'micare' ), get_the_date() );
        } elseif ( is_month() ) {
            $title = sprintf( esc_html__( 'Monthly: %s', 'micare' ), get_the_date( 'F Y' ) );
        } elseif ( is_year() ) {
            $title = sprintf( esc_html__( 'Yearly: %s', 'micare' ), get_the_date( 'Y' ) );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'micare' ), get_search_query() );
